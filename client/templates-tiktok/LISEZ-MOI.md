# Dépôt d'images pour le pack "TikTok France"

Dépose ici les images à ajouter au pack, puis lance depuis la racine du dépôt :

```bash
npm run templates:tiktok --workspace client
```
...
4. Elle ajoute les entrées correspondantes dans `client/src/lib/packs/tiktok.ts`.
...
`rio-de-janeiro.jpg` → template nommé « Rio de janeiro », d'id
`tiktok-rio-de-janeiro`. Donne donc des noms de fichiers lisibles : ils
s'affichent tels quels dans l'éditeur de zones, et deux templates ne peuvent
